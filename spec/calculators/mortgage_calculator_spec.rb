...
RSpec.describe MortgageCalculator, type: :model do
  ...
  describe "custom validations" do
    let(:calculator) { MortgageCalculator.new(property_price: 100000, down_payment: 5000, annual_interest_rate: 5, amortization_period: 25, payment_frequency: "monthly") }

    it "should add error if down payment is greater than property price" do
      calculator.down_payment = 100001
      calculator.valid?
      expect(calculator.errors[:down_payment]).to include("can't be greater than property price")
    end
